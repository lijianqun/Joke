...
public class TableUtils {
	 ...
	 * key: entityType.name
	 */
	private static ConcurrentHashMap<String, com.jamtu.db.library.db.table.Id> entityIdMap = new ConcurrentHashMap<String, com.jamtu.db.library.db.table.Id>();

	/* package */
	static synchronized com.jamtu.db.library.db.table.Id getId(Class<?> entityType) {
		if (Object.class.equals(entityType)) {
		...
		com.jamtu.db.library.db.table.Id id = new com.jamtu.db.library.db.table.Id(entityType, primaryKeyField);
		entityIdMap.put(entityType.getName(), id);
		return id;
	}

	private static String getPrimaryKeyFieldName(Class<?> entityType) {
		com.jamtu.db.library.db.table.Id id = getId(entityType);
